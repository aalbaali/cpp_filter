...
std::vector<PoseEstimate> GetSe2InekfEstimates(PoseEstimate meas_prior,
  ...
  for (size_t k = 1; k < K; k++) {
    ...
    LieAlg u_km1(u_v_km1[0], u_v_km1[1], u_g_km1[0]);
    ...
    // Correction
    if (idx_gps < meas_gps.size() &&
        (meas_gps[idx_gps].time() <= X_hat[k].time())) {
      ...
      // Compute S_k
      JacYgps_nk S_k =
          H_k * P_k * H_k.transpose() + M_k * R_k * M_k.transpose();
    ...
    // Note: switch covariance type from covariance on [position; theta] to
    // [theta; position] (to do the analysis)
...
GetSe2InekfEstimates(const std::string filename_config) {
  ...
  // Assumptions:
  //      - Assumes the files include GPS measurements as exteroceptive
  //      measurements.
  ...
  std::cout << "Reading .yml configuration from '" << filename_config << "'"
            << std::endl;
#endif
  ...
  return GetSe2InekfEstimates(meas_prior, meas_gyro, meas_vel, meas_gps);
}
